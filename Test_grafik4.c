...
void rita_spelbrädda(void){
    ...
    int spelbräddanx = 64;
    for (int i = 0; i < 6; i++){
        for ( y = spelbräddany + i * SNAKE_PIXEL * 2; y < spelbräddany + SNAKE_PIXEL*(1+i*2); y++){
            for (int j = 0; j < 6; j++){
                for ( x = spelbräddanx + j * SNAKE_PIXEL * 2; x < spelbräddanx + SNAKE_PIXEL*(1+j*2); x++){
                    unsigned char röd_stor = 155;
                    unsigned char grön_stor = 87;
                    unsigned char blå_stor = 13;
                    färga_pixel(x, y, röd_stor, grön_stor, blå_stor);
                }
                for ( x = spelbräddanx + SNAKE_PIXEL *(1+j*2); x < spelbräddanx + SNAKE_PIXEL*(2+j*2); x++){
        ...
        for ( y = spelbräddany + SNAKE_PIXEL *(1+i*2); y < spelbräddany + SNAKE_PIXEL*(2+i*2); y++){
            for (int j = 0; j < 6; j++){
                for ( x = spelbräddanx + j * SNAKE_PIXEL * 2; x < spelbräddanx + SNAKE_PIXEL*(1+j*2); x++){
                    unsigned char röd_stor = 117;
                    unsigned char grön_stor = 66;
...
/*tar in en sökning efter en mall till äpplet och färgar ett område utifrån en angiven position med 
hjälp av den färg som angivits. Så länge pixeln som ska färgas finns på skärmen. */
void rita_maskhuvud( int y, int x, int index, unsigned char röd_stor, unsigned char grön_stor, unsigned char blå_stor, int skala) {
    int rad, kolumn;
    unsigned int linje;
    unsigned char färg = packa_färg(röd_stor, grön_stor, blå_stor);
                ...
                for (int dy = 0; dy < skala; dy ++){
                    for ( int dx = 0; dx < skala; dx ++){
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            int px = y + SNAKE_PIXEL - rad * skala +dy;
                            int py = x  +  kolumn * skala + dx;
                            if (px >= 0 && px < VGA_BREDD && py >= 0 && py < VGA_HÖJD){
                                volatile unsigned char *skärm_pekare = skräm_skrivare(py * VGA_BREDD +px);
                                *skärm_pekare = färg;
                                }
                        ...
                        case 3:
                            int px = x + kolumn * skala + dx;
                            int py = y + SNAKE_PIXEL - rad * skala +dy;
                            if (px >= 0 && px < VGA_BREDD && py >= 0 && py < VGA_HÖJD){
                                volatile unsigned char *skärm_pekare = skräm_skrivare(py * VGA_BREDD +px);
                                *skärm_pekare = färg;
                                }
...
/*Anger vilka färger som ska färga vilken mall till äpplet.*/
void färga_maskhuvud( int x, int y, int skala){
    rita_maskhuvud(x, y, 0, 255, 163, 177, skala);
    rita_maskhuvud(x, y, 1, 216, 106, 122, skala);

}

void rita_maskkropp(int maskkroppx, int maskkroppy){
    int x, y;
    for ( y = maskkroppy; y < maskkroppy + SNAKE_PIXEL; y++){
        for(x = maskkroppx; x < maskkroppx + SNAKE_PIXEL; x++){
    ...
    for ( y = maskkroppy + 1; y < maskkroppy + SNAKE_PIXEL - 1; y++){
        for(x = maskkroppx; x < maskkroppx + SNAKE_PIXEL; x++){
...
void färga_maskkropp(){
    if (riktning ==1|| riktning ==3){
        rita_maskkropp( maskenssvansx[0],maskenssvansy[0]);
    }
    else if (riktning==2|| riktning==4){
        rita_maskkropp( maskenssvansy[0], maskenssvansx[0]);
    }
    for ( int i = 1; i < masklängd; i++){
        if ( maskenssvansx[i]== maskenssvansx[i-1] && maskenssvansy[i] != maskenssvansy [i-1]){
            rita_maskkropp( maskenssvansx[i],maskenssvansy[i]);
        }
        else if ( ( maskenssvansy[i]== maskenssvansy[i-1] && maskenssvansx[i] != maskenssvansx [i-1])){
            rita_maskkropp( maskenssvansy[i], maskenssvansx[i]);
        }
    }
}

void mask(){
    färga_maskhuvud(maskenshuvudx, maskenshuvudy, 1);
    färga_maskkropp();
}
